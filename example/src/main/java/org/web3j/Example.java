...
public class Example {
    ...
    void getTransactionExample() throws Exception {
        ...
        EthTransaction ethTransaction =
                web3j.ethGetTransactionByHash("0x2a4520eaa85fec4bfffae7e30da1b7d4f9e06a5bd21560ae8cf87bb65de64974").send();
        Transaction transaction = ethTransaction.getTransaction().get();

        System.out.println("Transaction: " + transaction.getHash() + " , from: " +
                transaction.getOgoFrom() + " , to: " + transaction.getOgoTo() + "\n");
    }

    void getBlockExample() throws Exception {
        System.out.println("Doing getBlockExample");
        EthBlock ethBlock =
                web3j.ethGetBlockByNumber(DefaultBlockParameter.valueOf(Numeric.toBigInt("0xA5624")), true).send();
        EthBlock.Block block = ethBlock.getBlock();
        List<Transaction>  transactions = block.getTransactions().stream()
                .map(transactionResult -> (Transaction) transactionResult.get())
                .collect(Collectors.toList());
        System.out.println("block " + block.getNumber() + ", has " + transactions.size() + " transactions.\n");
    ...
    void generateAddressExample() throws Exception {
        ...
        System.out.println("private key: " + sPrivatekeyInHex + " address:" + address + ", ogo address:" + addressOgo);
    }
    ...
    void createAndSignTransactionExample() throws Exception{
        ...
        RawTransaction rawTransaction  = RawTransaction.createOgoTransaction(
                nonce,
